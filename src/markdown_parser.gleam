...
fn process_line(state: ParseState, line: String) -> ParseState {
  ...
  case is_section_header(trimmed_line) {
    ...
    False -> case is_task_line(trimmed_line) {
      True -> {
        let state_with_item = add_current_item_to_completed(state)
        case parse_task_line(trimmed_line, state_with_item.current_section) {
          Some(new_item) -> ParseState(..state_with_item, current_item: Some(new_item))
          None -> state_with_item
        }
      }
      False -> case is_note_line(line) {
        True -> {
          case state.current_item {
            Some(item) -> {
              let note_text = extract_note_text(line)
              let updated_item = TodoItem(..item, notes: [note_text, ..item.notes])
              ParseState(..state, current_item: Some(updated_item))
            }
            None -> state
          }
        }
        False -> state  // Ignore other lines
      }
    }
  }
}
    ...
    Ok(re) -> {
      case regexp.scan(re, line) {
        [regexp.Match(_, [Some(checkbox), Some(content)])] -> {
          ...
          let #(summary, context, due_date, start_date) = parse_task_content(content)
...
fn parse_task_content(content: String) -> #(String, Option(String), Option(Time), Option(Time)) {
    ...
    Ok(re) -> {
      case regexp.scan(re, content) {
        [regexp.Match(_, [Some(summary), Some(context)])] -> 
          #(string.trim(summary), Some(context))
        _ -> #(content, None)
      }
    }
...
fn extract_start_date(content: String) -> #(String, Option(Time)) {
  let start_pattern = "^(.+?)\\s*-?\\s*[Ss]cheduled\\s+for\\s+(\\d{1,2}/\\d{1,2})\\s*$"
  ...
  case string.split(date_str, "/") {
    [month_str, day_str] -> {
      case int.parse(month_str), int.parse(day_str) {
        Ok(month), Ok(day) if month >= 1 && month <= 12 && day >= 1 && day <= 31 -> {
          // Use current year for now - simplified approach
          let _current_time = birl.utc_now()
          let current_year = 2024  // Simplified - use current year
          ...
          let date_string = int.to_string(current_year) <> "-" <> month_padded <> "-" <> day_padded
...
fn add_current_item_to_completed(state: ParseState) -> ParseState {
  case state.current_item {
    Some(item) -> {
      ...
      let item_with_correct_notes = TodoItem(..item, notes: list.reverse(item.notes))
      ParseState(
